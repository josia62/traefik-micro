...
// src/service/middleware/grpc.ts
var serverGrpc = new grpc.Server();
...
import { StatusCodes as StatusCodes2, getReasonPhrase } from "http-status-codes";
...
var customLogLevel = (_req, res, err) => {
  if (err || res.statusCode >= StatusCodes2.INTERNAL_SERVER_ERROR) return "error" /* Error */;
  if (res.statusCode >= StatusCodes2.BAD_REQUEST) return "warn" /* Warn */;
  if (res.statusCode >= StatusCodes2.MULTIPLE_CHOICES) return "silent" /* Silent */;
  return "info" /* Info */;
};
var customSuccessMessage = (req, res) => {
  if (res.statusCode === StatusCodes2.NOT_FOUND) return getReasonPhrase(StatusCodes2.NOT_FOUND);
...
var App = class {
  ...
  initRoutes = async () => {
    ...
    const { appRouter } = await import("./app.route-OTCCVRZV.mjs");
...
var bootstrapAsync = async () => {
  ...
  try {
    ...
    grpc_default.bindAsync(`localhost:${GRPC_PORT}`, grpc2.ServerCredentials.createInsecure(), (err, port) => {
      if (err) {
        logger.error(err);
        return;
      }
